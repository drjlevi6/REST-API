...
// This endpoint requires a JSON object as the body of the req argument;
//	returns student info (as a JSON	 object) including newly generated ID.
...
//   }
// }
// Response (raw): "{"name":"David Frankl","classes":{"biology":65,"algebra":77},
//	"id":"86ec711e-98c3-498e-9500-6130a1eda6f4"}"
...
app.delete('/students/:id', (req, res) => {
	...
	if (student) {
		...
		res.status(201).send('Student ' + req.params.id + 
			' was deleted.');
	}
});
...
app.put('/students/:name/:class/:grade', (req, res) => {
	...
	if (student) {
		...
		res.status(201).send('Student ' + req.params.name + 
			' was assigned a grade of ' + req.params.grade + 
			' in ' + req.params.class);
	} else {
		res.status(404).send('Student with the name ' + 
			req.params.name + ' was not found.');
	}
});
...
app.get('/students/:name/gpa', (req, res) => {
	...
	if (student) {
		/* Object.values() filters out object's keys and keeps the values 
		that are returned as a new array */
